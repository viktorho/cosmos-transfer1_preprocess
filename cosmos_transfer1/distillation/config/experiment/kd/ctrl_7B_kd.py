...
"""
This script will make + register the KD configs for all the control modalities (one config per modality).
...
Example usage:
    - [dryrun, generate and inspect edge control KD config]:
    ...
    - [real run, 8 gpu, train edge control KD from released checkpoint]:
...
def make_ctrl_kd_experiment(hint_key: str = "control_input_edge") -> dict:
    ...
    return dict(
        ...
        job=dict(
            ...
            name=f"DISTILL_CTRL_7Bv1_{hint_key_short}_fsdp_kd_train",
        ),
        checkpoint=dict(
            save_iter=500,
...
for key in CTRL_HINT_KEYS_COMB.keys():
    if key in ["control_input_upscale", "control_input_hdmap", "control_input_lidar"]:
        continue
    exp = make_ctrl_kd_experiment(key)
    cs.store(
        group="experiment",
        package="_global_",
        name=exp["job"]["name"],
